 ...
 * 
 * 1/30/17: stack_top adjusted down from 0x8000_0000 to 0x7ff0_0000
 *
...
//
//  riscv_mem: A dual-ported virtual word-addressed memory with 5 segments.
//             Performing any memory operation to a memory address while
//             that memory address is being written produces an undefined
//             result.
//
//  Lab 4 version: data_out1 returns 16 consecutive bytes without alignment restrictions
//
// To compile assembly files for the RISC-V, use the Makefile:
//
//    make assemble TEST=inputs/mytest.s
//
...
module riscv_mem(addr1,data_in1,data_out1,we1,excpt1,allow_kernel1,kernel1,
   ...
   output [31:0] data_out1[3:0];     // Memory read data
   reg [31:0]    data_out1[3:0];
   ...
   initial
     begin
    // Initialize Port 1
    data_out1[0] = 32'hxxxxxxxx;
    data_out1[1] = 32'hxxxxxxxx;
    data_out1[2] = 32'hxxxxxxxx;
    data_out1[3] = 32'hxxxxxxxx;
    excpt1 = 1'b0;
    kernel1 = 1'b0;
   ...
   always@(rst_b)
     if(rst_b == 1'b0)
      ...
      $readmemh("mem.text.hex", text_seg);
      $readmemh("mem.data.hex", data_seg);
      $readmemh("mem.ktext.hex", ktext_seg);
      $readmemh("mem.kdata.hex", kdata_seg);
       end
    ...
    if((write_mask1 != 32'b0) && (rst_b == 1'b1))
      ...
      begin
         ...
         if((addr1 >= text_start_w) && (addr1 < text_top_w))
          ...
          excpt1 <= 1'b0;
          kernel1 <= 1'b0;
          data_out1[0] <= text_seg[addr1 - text_start_w];
          data_out1[1] <= text_seg[addr1 + 1 - text_start_w];
          data_out1[2] <= text_seg[addr1 + 2 - text_start_w];
          data_out1[3] <= text_seg[addr1 + 3 - text_start_w];
           end
         else if((addr1 >= data_start_w) && (addr1 < data_top_w))
           begin
          excpt1 <= 1'b0;
          kernel1 <= 1'b0;
          data_out1[0] <= data_seg[addr1 - data_start_w];
          data_out1[1] <= data_seg[addr1 + 1 - data_start_w];
          data_out1[2] <= data_seg[addr1 + 2 - data_start_w];
          data_out1[3] <= data_seg[addr1 + 3 - data_start_w];
           end
         else if((addr1 >= stack_start_w) && (addr1 < stack_top_w))
           begin
          excpt1 <= 1'b0;
          kernel1 <= 1'b0;
          data_out1[0] <= stack_seg[addr1 - stack_start_w];
          data_out1[1] <= stack_seg[addr1 + 1 - stack_start_w];
          data_out1[2] <= stack_seg[addr1 + 2 - stack_start_w];
          data_out1[3] <= stack_seg[addr1 + 3 - stack_start_w];
           end
         else if((addr1 >= ktext_start_w) && (addr1 < ktext_top_w))
           begin
          kernel1 <= 1'b1;
          if(allow_kernel1 == 1'b1)
            begin
               excpt1 <= 1'b0;
               data_out1[0] <= ktext_seg[addr1 - ktext_start_w];
               data_out1[1] <= ktext_seg[addr1 + 1 - ktext_start_w];
               data_out1[2] <= ktext_seg[addr1 + 2 - ktext_start_w];
               data_out1[3] <= ktext_seg[addr1 + 3 - ktext_start_w];
            end
          else
            begin
               excpt1 <= 1'b1;
          ...
          if(allow_kernel1 == 1'b1)
            begin
               excpt1 <= 1'b0;
               data_out1[0] <= kdata_seg[addr1 - kdata_start_w];
               data_out1[1] <= kdata_seg[addr1 + 1 - kdata_start_w];
               data_out1[2] <= kdata_seg[addr1 + 2 - kdata_start_w];
               data_out1[3] <= kdata_seg[addr1 + 3 - kdata_start_w];
            end
          else
            begin
               excpt1 <= 1'b1;
